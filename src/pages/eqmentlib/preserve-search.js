app.controller("searchCtrl", ["$scope", "ApiService", "DataService", "UtilsService", "Loading", "CacheService", "AuthService", "RPCObserver", "$timeout", function($scope, ApiService, DataService, UtilsService, Loading, CacheService, AuthService, RPCObserver, $timeout) {
	...
	$scope.search = {
		...
		history: [],
		getHistory: function() {
			var temp = CacheService.get(userId + "equ_preserve_search");
			if(temp)
				$scope.search.history = CacheService.get(userId + "equ_preserve_search");
			$scope.domShowX = true;
			$scope.domShow = false;
		},
		doSearch: function() {
			...
			CacheService.set(userId + "equ_preserve_search", $scope.search.history, true);
			//开启上拉事件
	...
	function getPagedCompEqOverhaulList() {
		var url = ApiService.Api50 + "/api/v1/Equipment/GetPagedCompEqMaintainList?pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSize + "&eqIdentifier=" + encodeURIComponent($scope.search.text) + "&authType=0";
