...
(function($, window) {
	...
	var imgId = 0;
		var startEvent;
	var endEvent;
	var PreviewImage = function(options,start, end) {
		startEvent = start;
		endEvent = end;
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('webkitAnimationStart', function() {
	...
	proto.dispose = function() {
		var prevdom = document.geproto.disposelementById("__MUI_PREVIEWIMAGE");
		prevdom && prevdom.parentNode.removeChild(prevdom);
		previewImageApi = null;
	};
	var previewImageApi = null;
	$.previewImage = function(options,start, end) {
		if (!previewImageApi) {
			previewImageApi = new PreviewImage(options,start, end);
		}
