app.controller("ContactOrgController", ["$scope", "ApiService", "DataService", "UtilsService", "Loading", "AuthService", function ($scope, ApiService, DataService, UtilsService, Loading, AuthService) {
	//部门id
	$scope.OrgID = 0;
	//页面数据
	$scope.data = {};
	...
	$scope.doSearchUser = function () {
		UtilsService.openWindow({
			id: "search-user.html",
			url: "search-user.html?fromType=contact"
		});
	}
	//刷新列表信息
	...
	RpcServer.expose("RPC_ReloadCurListData", function (params) {
		$scope.OrgID = 0;
		$scope.navDirData = [{
			ID: 0,
			OrgName: "组织架构"
