		...
		mui.init({
			...
			beforeback: function () {
				...
				RpcClient.invoke("message.html", "RPC_Draft", {
						ID: dom.properties.scope.chatData.GroupCode,
						isDraft: isDraft
					}),
...
app.controller("chatController", ["$scope", "$sce", "ApiService", "DataService", "CacheService", "UtilsService",
	...
	function ($scope, $sce, ApiService, DataService, CacheService, UtilsService, Loading, BillService, $filter,
		...
		$scope.chatData.Api50 = dom.properties.apiHost;
...
var dom = {
	...
	webView: {
		...
		initParams: function () {
			...
			dom.properties.scope.chatData.GroupCode = this.cur.GroupCode;
			dom.properties.scope.chatData.GroupName = this.cur.GroupName;
		...
		more: {
			properties: {
				path: "../menus/menu-chat-group.html",
				id: "menu-chat-group.html",
			},
			obj: "",
			init: function () {
				var _chatData = dom.properties.scope.chatData;
				this.properties.id = this.properties.id + _chatData.GroupCode;
				...
				if (!this.obj) {
					...
					this.obj = plus.webview.create(this.properties.path, this.properties.id, styles, {
						GroupName: _chatData.GroupName,
						GroupCode: _chatData.GroupCode,
						needLogin: true
					});
				}
			},
		...
		service: {
			...
			ReadMessage: function () {
				RpcClient.invoke("message.html", 'RPC_ReadMsg', dom.properties.scope.chatData.GroupCode);
			},
			Rpc_refreshMemberName: function () {
				RpcServer.expose("Rpc_refreshMemberName", function (_data) {
					dom.properties.scope.chatData.GroupUsers[_data.UserID].MemberName = _data.MemberName;
					dom.properties.scope.$apply();

					angular.forEach(document.querySelectorAll(".mui-right.md-userCustomLogo"), function (_item) {
						_item.innerText = _data.MemberName.substr(0, 1);
					});

					angular.forEach(document.querySelectorAll(".msg-content-memberName[data-user='" + _data.UserID + "']"),
						function (_item) {
							_item.innerText = _data.MemberName;
						});
				});
			},
			//刷新群组
			Rpc_RefGroup: function () {
				RpcServer.expose("Rpc_RefGroup", function (data) {
					//刷新用户列表
					if (data.Type == 0) {
						dom.content.pushTips(data.UserID, data.MemberName, "退出了本群");
					} else {
						dom.content.pushTips(data.UserID, data.MemberName, "加入了本群");

						dom.properties.scope.chatData.GroupUsers[data.UserID] = {
							UserID: data.UserID,
							MemberName: data.MemberName
						};

						curData.chatData.getGroupUsrs();
					}
				});
			},
	...
	subView: {
		...
		exption: {
			...
			tap: function () {
				mui(".exption-view").on("tap", "li.mui-media", function () {
	...
	msg: {
		send: function (type, content) {
			...
			var packData = {
				GroupCode: _chatData.GroupCode,
				GroupName: _chatData.GroupName,
				reName: _chatData.GroupUsers[_chatData.UserID].MemberName || _chatData.GroupUsers[_chatData.UserID].ViewName ||
					_chatData.UserName,
				content: content,
				userId: _chatData.UserID,
				userName: _chatData.GroupUsers[_chatData.UserID].MemberName || _chatData.GroupUsers[_chatData.UserID].ViewName ||
					_chatData.UserName,
				type: type
			};

			var obj = transMessage.packGroupSimple(packData);
		...
		push: function (msg) {
			...
			if (msg.SendName && msg.SendName != (dom.properties.scope.chatData.GroupUsers[msg.SendID].MemberName || dom.properties.scope.chatData
					.GroupUsers[msg.SendID].ViewName)) {
				dom.properties.scope.chatData.GroupUsers[msg.SendID].MemberName = msg.SendName;
				angular.forEach(document.querySelectorAll(".md-userCustomLogo[data-user='" + msg.SendID + "']"), function (_item) {
					_item.innerText = msg.SendName.substr(0, 1);
				});
				angular.forEach(document.querySelectorAll(".msg-content-memberName[data-user='" + msg.SendID + "']"), function (
					_item) {
					_item.innerText = msg.SendName;
				});
			}
			dom.content.scrollToBottom();
		...
		sendFileToService: function (id, content, fileType, extParams) {
			...
			if (msgItemDom) {
				...
				var sendData = {
					MsgID: msgItem.MsgID,
					ReceiveID: msgItem.GroupCode,
					GroupName: msgItem.GroupName,
					GroupCode: msgItem.GroupCode,
	...
	content: {
		backHtml: "<div class='msg-item-tips'><p><span class='user-name'>${1}</span>已撤回消息</p></div>",
		...
		tap: function () {
			...
			}).on("tap", ".open-user", function (e) {
				var id = this.getAttribute("data-id");

				var viewUrl = "../contact/personal-info.html?userid=" + id;
				var viewId = "personal-info.html";
			...
			tap: function () {
				mui(".mui-scroll-wrapper").on("tap", ".resend", function (event) {
					...
					switch (type) {
						...
						case "5":
							var content = JSON.parse(msgItem.Content);
							dom.msg.sendFile(msgItem.UpLoadCallBack || content[1][0][3], msgItem.MsgID, msgItem.IsUpload, "attachment");
							break;
						case "6":
							var content = JSON.parse(msgItem.Content);
		...
		itemEvent: {
			...
			domEvent: function () {
				mui(".mui-scroll-wrapper").on("tap", ".content-event", function (e) {
					...
					e.preventDefault();
					e.stopPropagation();
				}).on("tap", ".at-text", function () {
					var content = '<span class="at-text" contenteditable="false" >' + this.innerHTML + '</span> ';
					dom.footer.input.dom.innerHTML = (dom.footer.input.dom.innerHTML += content);
					mui.trigger(dom.footer.input.dom, 'input');
					...
					if (!_this.classList.contains("msg-item-self")) {
						document.querySelector("#ac-aite").style.display = "block";
					}
					mui('#sheet1').popover('toggle');
				});
				...
				document.querySelector("#ac-aite").addEventListener("tap", function () {
					mui('#sheet1').popover('toggle');

					var msgId = (dom.msg.activeData.ID ? dom.msg.activeData.MsgID + "-" + dom.msg.activeData.Version : dom.msg.activeData.MsgID);
					var memberName = document.querySelector("#msg-item-" + msgId + " .msg-content-memberName").innerText;

					var content = '<span class="at-text" contenteditable="false" >@' + memberName + '</span> ';
				...
				document.querySelector("#ac-withdraw").addEventListener("tap", function () {
					...
					var backData = {
						...
						vBack: "RPC_MsgBack",
						SendID: dom.properties.scope.chatData.UserID
					};
				...
				document.querySelector("#ac-down").addEventListener("tap", function () {
					...
					if (content[1][0][0] == 7) {
						...
						dom.content.downLoadFile(fileName, content[1][0][5][2], dataFile, isDown, msgId);
					} else {
						var dataFile = content[1][0][3];
						if (dataFile.indexOf("NW") >= 0) {
							mui.toast("请在迈迪设计宝查看！");
		...
		hideAc: function () {
			...
			document.querySelector("#ac-aite").style.display = "none";
		},
		...
		pushTips: function (userId, userName, tips) {

			var content = angular.element("<div class='msg-item'>")

			var nowDate = Date.now();
			var scope = dom.properties.scope;
			var _msgList = scope.chatData.msgList;

			var prev = _msgList[_msgList.length - 1].SendTime;

			prev = new Date(prev.replace(/-/g, "/")).getTime();

			if (nowDate - prev > 60 * 1000) {
				var time_split = angular.element("<div class='time-split'>").html("<span>" + scope.event.getDate(nowDate) +
					"</span>");
				content.append(time_split);
			}

			var _element = angular.element("<div class='msg-item-tips'><p><span class='user-name open-user' data-id='" +
				userId + "'>" + userName + "</span><span class='nomarl'>" + tips + "</span></p></div>");
			content.append(_element);

			angular.element(dom.content.msgList).append(content);

			_msgList.push({
				SendTime: dom.properties.filter("dateFormat")(nowDate, "yyyy-MM-dd HH:mm:ss")
			})
			setTimeout(function () {
				dom.content.scrollToBottom();
			}, 200);
...
var curData = {
	chatData: {
		...
		load: function () {
			...
			var url = dom.properties.apiHost + "/api/v1/Message/MsgGroupRecord_Pull?groupCode=" + scope.chatData.GroupCode +
				"&pageSize=" + this.count + "&curId=" + this.curId + "&pullType=1&isRead=1";
		...
		setRead: function () {
			var scope = dom.properties.scope;
			var url = dom.properties.apiHost + "/api/v1/Message/MsgRecord_Group_Read?groupCode=" + scope.chatData.GroupCode;
		...
		getGroupInfo: function () {
			var _this = this;
			var url = dom.properties.apiHost + "/api/v1/Message/GroupInfo?groupCode=" + dom.properties.scope.chatData.GroupCode;
			...
			dom.properties.DataService.get(url).then(function (data) {
				...
				RpcClient.invoke("message.html", "Rpc_GroupSilence", {
					GroupCode: chatData.GroupCode,
					IsSilence: chatData.GroupInfo.IsSilence
				});

				curData.chatData.getGroupUsrs(function () {
		...
		getGroupUsrs: function (callback) {
			var url = dom.properties.apiHost + "/api/v1/Message/GroupMemberList_His?groupId=" + dom.properties.scope.chatData.GroupInfo
				.ID;
			dom.properties.DataService.get(url).then(function (data) {
		...
		userPhoto: function (_data) {
			...
			var user = scope.chatData.GroupUsers[_data.SendID];
			if (!user) {
				user = {
					UserID: _data.SendID,
					MemberName: _data.SendName,
					ULogoIsExist: false
				};
			}

			_photo = angular.element("<div class='msg-user-img'>").addClass(user.UserID == scope.chatData.UserID ? "mui-right" :
				"");
			_photo.attr("data-user", _data.SendID);

			if (user.ULogoIsExist) {
				_photo.css("background-image", "url(" + filter("getUserLogo2")(user.UserID, 100, 100) + ")");
			} else {
				_photo.addClass("md-userCustomLogo");
				_photo.html(filter("getUserCustomLogo")(user.MemberName || user.ViewName));
			}
			return _photo;
		},
		getListHtml: function (prev, data, sortType) {
			...
			var elements = data.map(function (_message, _index) {
				var _photo = _this.userPhoto(_message);
				...
				if (_message.ID && _message.MsgType == 25) {
					var userInfo = scope.chatData.GroupUsers[_message.SendID];
					var backHtml;
					if (userInfo) {
						backHtml = dom.content.backHtml.replace("${1}", (userInfo.MemberName || userInfo.ViewName));
					} else {
						backHtml = dom.content.backHtml.replace("${1}", (_message.SendName || ""));
					}
					_content.append((is_self ? dom.content.backHtmlSlef : backHtml));
				} else {

					_content.append((_photo).clone());
					...
					_content.append(angular.element('<div class="msg-content-memberName mui-ellipsis" data-user="' + _message.SendID +
						'">' + (scope.chatData.GroupUsers[_message.SendID].MemberName || scope.chatData.GroupUsers[_message.SendID].ViewName) +
						'</div>').addClass(_message.SendID == scope.chatData.UserID ? "msg-content-memberName-right" : ""));
		...
		reload: function () {
			...
			dom.properties.scope.chatData.GroupUsers = {};
			curData.chatData.getGroupInfo();
		},
		reloadNoRead: function () {
			...
			var url = dom.properties.apiHost + "/api/v1/Message/MsgGroupRecord_NoRead?groupCode=" + scope.chatData.GroupCode + "&pullType=1&sendTime=" + curData.chatData.noReadLoad;;
				...
				data.Back.forEach(function (item) {
					...
					if (_dom) {
						if (_dom.classList.contains("msg-item-self")) {
							...
							var backHtml = dom.content.replace('${1}', _dom.querySelector(".msg-content-memberName").innerText);
							_dom.querySelector(".msg-item-body").innerHTML = backHtml;
						}
					}
				});
	...
	backMsg: function (msg) {
		...
		if (msg.msgClientNo) {
			RpcClient.invoke("message.html", "RPC_MsgBackCall", {
				...
				OppoID: dom.properties.scope.chatData.GroupCode,
				SendID: msg.SendID
			})
			...
			var userInfo = dom.properties.scope.chatData.GroupUsers[msg.SendID];
			var backHtml;
			if (userInfo) {
				backHtml = dom.content.backHtml.replace("${1}", (userInfo.MemberName || userInfo.ViewName));
			} else {
				backHtml = dom.content.backHtml.replace("${1}", (""));
			}

			_dom.innerHTML = backHtml;

		} else {
			var url = dom.properties.ApiService.Api50 + "/api/v1/Message/RevokeMsg?msgId=" + clientNos[0] + "&version=" + clientNos[1] + "&groupId=" + dom.properties.scope.chatData.GroupCode;
			dom.properties.DataService.get(url).then(function () {
				...
				var _msg = transMessage.packGroupSimple(packData, '[0,[[1,"#WITHDRAW_MSGID_' + clientNos[0] + '|' + clientNos[1] + '"]]]');
