...
var mapObj = {
	...
	tools: {
		marker: {
			...
			getMarker: function(data, position) {
				var content = "<div class='point_comp'><div class='div_count' >" + (data.Count > 99 ? "99+" : data.Count) + "</div></div>";
				...
				var marker = new AMap.Marker({
					...
					icon: new AMap.Icon({
						...
						image: "../../images/industry/point-bluex2.png",
						imageSize: new AMap.Size(53, 58),
						imageOffset: new AMap.Pixel(2, 2)
					}),
					offset: new AMap.Pixel(-28, -55)
				});
	...
	ui: {
		...
		markerList: {
			...
			selected: null,
			//获取普通节点的样式
			getSimplePoint: function(id) {
				var label = {
					offset: new AMap.Pixel(10, 10),
					content: "<div class='point_comp'><img class='simplePoint' src='http://pic.maidiyun.com/Y29tcC9sb2dv_" + id + "_43x43.png'/></div>"
				};
				...
				new AMap.Icon({
					...
					image: "../../images/industry/point_orage.png",
					imageSize: new AMap.Size(80, 87),
					imageOffset: new AMap.Pixel(2, 2)
				});

				var offset = new AMap.Pixel(-28, -55);
				return [label, icon, offset];
			},
			//获取选中节点的样式
			getSelectedPoint: function(id) {
				...
				var label = {
					offset: new AMap.Pixel(10, 10),
					content: "<div class='point_comp'><img class='bigPoint' src='http://pic.maidiyun.com/Y29tcC9sb2dv_" + id + "_69x69.png'/></div>"
				};
				var offset = new AMap.Pixel(-41, -85);
				return [label, icon, offset];
			},
			load: function() {
				AMapUI.loadUI(['misc/MarkerList'], function(MarkerList) {
					mapObj.ui.markerList.obj = new MarkerList({
						...
						getMarker: function(dataItem, context, recycledMarker) {
							...
							var markerData = mapObj.ui.markerList.getSimplePoint(dataItem.CompID);
					...
					mapObj.ui.markerList.obj.on("renderComplete", function() {
						...
						if(mapObj.ui.markerList.autoView) {
							mapObj.map.setFitView(mapObj.ui.markerList.obj.getAllMarkers());
			...
			selectedData: function(selectedData) {
				...
				if(mapObj.ui.markerList.selected) {
					var markerData = mapObj.ui.markerList.getSimplePoint(mapObj.ui.markerList.selected.data.CompID);

					mapObj.ui.markerList.selected.marker.setLabel(markerData[0]);
					mapObj.ui.markerList.selected.marker.setIcon(markerData[1]);
					mapObj.ui.markerList.selected.marker.setOffset(markerData[2]);
				...
				var markerData = mapObj.ui.markerList.getSelectedPoint(selectedData.data.CompID);
				mapObj.ui.markerList.selected.marker.setLabel(markerData[0]);
				mapObj.ui.markerList.selected.marker.setIcon(markerData[1]);
				mapObj.ui.markerList.selected.marker.setOffset(markerData[2]);
			...
			renderData: function(data) {
				if(data.length > 30 && mapObj.ui.markerList.autoView) {
					mapObj.ui.markerList.autoView = false;
				}
				...
				mapObj.ui.markerList.clearData();
				mapObj.ui.markerList.obj.render(data);
			}
		}
	},
	//坐标转换
...
var curData = {
	...
	loadMapData: function() {
		if(curData.MapData[vm.curPosition.city] && !vm.search.key && vm.search.class.length == 0) {
	...
	loadMapDataToService: function() {
		...
		var url = MdAppUrl.Api45 + "/api/v1.1/EpDemand/GetCompMap?mapLng=" + vm.curPosition.position[0] + "&mapLat=" + vm.curPosition.position[1] + "&province=" + vm.curPosition.province + "&city=" + vm.curPosition.city + "&district=" + vm.curPosition.district + "&key=" + vm.search.key + "&path=" + paths.join(",");
		getAjaxData(url, function(res) {
			if(res && res.State > 0) {
				if(!vm.search.key && vm.search.class.length == 0) {
	...
	loadMapCount: function() {
		if(curData.MapData["count"] && !vm.search.key && vm.search.class.length == 0) {
	...
	loadMapCountToService: function() {
		...
		var url = MdAppUrl.Api45 + "/api/v1.1/EpDemand/GetCompMapCount?key=" + vm.search.key + "&path=" + paths.join(",");
		getAjaxData(url, function(res) {
			if(res && res.State > 0) {
				if(!vm.search.key && vm.search.class.length == 0) {
