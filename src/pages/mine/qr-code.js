app.controller("myEwmDetailsCtrl", ["$scope", "CacheService", "UtilsService", "ApiService", "AuthService", "$timeout", "$filter", "ChatUserService", function ($scope, CacheService, UtilsService, ApiService, AuthService, $timeout, $filter, ChatUserService) {
	...
	AuthService.getAuth().then(function (res) {
		$scope.myDetailsVM.CompName = res.CompName;
		$scope.myDetailsVM.ViewName = $filter("getViewName")(res.UserName, res.RealName, res.Mdt);
		$scope.myDetailsVM.Mdt = res.Mdt;
		$scope.myDetailsVM.UserImg = ApiService.pic + "/dXNlci9sb2dv_" + res.UserID + "_45x45.png";
		$scope.myDetailsVM.Sex = res.Sex;
		$scope.myDetailsVM.UserID = res.UserID;
		$scope.myDetailsVM.ULogoIsExist = res.ULogoIsExist;

		$("#qrcode").qrcode({
			render: 'canvas',
			text: utf16to8("http://m.maidiyun.com?MDU" + $scope.myDetailsVM.UserID),
			width: 180,
			height: 180,
			src: $scope.myDetailsVM.UserImg
		});
		$("#qrcode").show();
	});
