			...
			mui(".search-box")
				.on("tap", ".search-placeholder", function () {
					...
					dom.properties.UtilsService.openWindow({
						id: "chat-group-record-search.html",
						url: "chat-group-record-search.html",
						extras: {
							GroupCode: dom.properties.scope.chatData.GroupCode,
							GroupName: dom.properties.scope.chatData.GroupName
						}
					});
...
var curData = {
	chatData: {
		...
		load: function () {
			...
			var url = dom.properties.apiHost + "/api/v1/Message/MsgGroupRecord_Pull?groupCode=" + scope.chatData.GroupCode + "&pageSize=" + _this.count + "&curId=" + _this.preId + "&pullType=1";

			// else
			if (_this.canDragLoadBT && _this.nextId && _this.isLoaddingBT) {
				url = dom.properties.apiHost + "/api/v1/Message/MsgGroupRecord_Pull?groupCode=" + scope.chatData.GroupCode + "&pageSize=" + _this.count + "&curId=" + (_this.nextId - 1) + "&pullType=2";
				loadType = "next";
			}
		...
		userPhoto: function (_data) {
			...
			if (user.ULogoIsExist) {
				_photo.css("background-image", "url(" + filter("getUserLogo")(user.UserID, 50, 0) + ")");
			} else {
		...
		getListHtml: function (prev, data) {
			...
			var elements = data.map(function (_message) {
				...
				if (_message.ID && _message.MsgType == 25) {
					...
					_content.append(angular.element('<div class="msg-content-memberName mui-ellipsis" data-user="' + _message.SendID + '">' + (scope.chatData.GroupUsers[_message.SendID].MemberName || scope.chatData.GroupUsers[_message.SendID].ViewName) + '</div>').addClass(_message.SendID == scope.chatData.UserID ? "msg-content-memberName-right" : ""));
		...
		getGroupUsrs: function () {
			var url = dom.properties.apiHost + "/api/v1/Message/GroupMemberList_His?groupId=" + dom.properties.scope.chatData.GroupInfo.ID;
			dom.properties.DataService.get(url).then(function (data) {
