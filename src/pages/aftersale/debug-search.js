app.controller("searchCtrl", ["$scope", "ApiService", "DataService", "UtilsService", "Loading", "CacheService", "AuthService", "RPCObserver", "$timeout", function($scope, ApiService, DataService, UtilsService, Loading, CacheService, AuthService, RPCObserver, $timeout) {
	...
	$scope.search = {
		...
		history: [],
		getHistory: function() {
			var temp = CacheService.get(userId + "_debug_search");
			if(temp)
				$scope.search.history = CacheService.get(userId + "_debug_search");
			$scope.domShowX = true;
			$scope.domShow = false;
		},
		doSearch: function() {
			...
			CacheService.set(userId + "_debug_search", $scope.search.history, true);
			//开启上拉事件
	...
	function getCompRepairRecordsList() {
		var url = ApiService.Api50 + "/api/v1/ProdDebug/GetCompProdDebugList?keyWords=" + encodeURIComponent($scope.search.text)
		+ "&pageIndex=" + $scope.pageIndex + "&pageSize="
		+ $scope.pageSize + "&paramTypeId=" + paramsID + "&authType=" + query("authType")
	...
	RPCObserver.on('refresh_debug_search_list', 'refresh_debug_search_list');
	window.refresh_debug_search_list = function() {
		$scope.search.doSearch();
	};
}]);
