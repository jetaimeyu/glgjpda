...
app.controller("bodyCtl", ["$scope", "DataService", "ApiService", "UtilsService", "$filter", "TapService", "AuthService", "CacheService", "DatePickerService", function($scope, DataService, ApiService, UtilsService, $filter, TapService, AuthService, CacheService, DatePickerService) {
	...
	var curData = {
		title: function() {
			...
			var url = ApiService.Api50 + "/api/v1/Equipment/GetEqLogByDate?mdCode="+$scope.domData.mdcode+"&startDate=" + startDate.Format("yyyy-MM-dd 00:00:00") + "&endDate=" + endDate.Format("yyyy-MM-dd 00:00:00") + "&dataType=1&groupType=" + (curDate.curType != 'month' ? 1 : 0)+"&userId="+userId
		...
		data: function(key) {
			...
			var url = ApiService.Api50 + "/api/v1/Equipment/GetEqLogByDate?mdCode="+$scope.domData.mdcode+"&startDate=" + startDate.Format("yyyy-MM-dd 00:00:00") + "&endDate=" + endDate.Format("yyyy-MM-dd 00:00:00") + "&dataType=0&groupType=" + (curDate.curType != 'month' ? 1 : 0)+"&key="+key+"&userId="+userId
			DataService.get(url).then(function(res) {
				mui.hideLoading();
