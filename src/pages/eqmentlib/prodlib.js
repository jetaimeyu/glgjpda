app.controller("CompProdLibController", ["$scope", "ApiService", "DataService", "Loading", "$q", "UtilsService", "TapService", "$filter",function($scope, ApiService, DataService, Loading, $q, UtilsService, TapService,$filter) {
	...
	$scope.ProdDetails = function(id, compId) {
		UtilsService.openWindow({
			id:"eqmentlib/prodView.html",
			url:"prodView.html?id="+id+"&compId="+compId
		});
	}
	init();
	...
	$scope.loadDir = function() {
		var _defer = $q.defer();
		var url = ApiService.Api45 + "/api/v1.0/Equipment/GetCompEqCatalogList?compId=" + $scope.compId;
	...
	function loadProduct() {
		var url = ApiService.Api45 + "/api/v1.0/Equipment/GetCompEqInfoMergerList?page="+pageIndex+"&pageSize="+pageSize+"&compId="+ $scope.compId+"&catalogId="+(sid?sid:0);
		DataService.get(url).then(function(res) {
			$scope.isLoad = true;
