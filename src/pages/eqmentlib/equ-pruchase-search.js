app.controller("searchCtrl", ["$scope", "ApiService", "DataService", "UtilsService", "Loading", "CacheService", "AuthService", "RPCObserver", "$timeout", function($scope, ApiService, DataService, UtilsService, Loading, CacheService, AuthService, RPCObserver, $timeout) {
	...
	$scope.search = {
		...
		history: [],
		getHistory: function() {
			var temp = CacheService.get(userId + "_pruchase_equ_search");
			if(temp)
				$scope.search.history = CacheService.get(userId + "_pruchase_equ_search");
			$scope.domShowX = true;
		},
		doSearch: function(type) {
			...
			if(!$scope.state||type!=1) {
				...
				CacheService.set(userId + "_pruchase_equ_search", $scope.search.history, true);
			}
	...
	function GetEqInfoList() {
		var url = ApiService.Api52 + "/api/v1/EqApply/GetEqPurchasePageList?catalogID=0&pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSize + "&eqName=" + encodeURIComponent($scope.search.text);
