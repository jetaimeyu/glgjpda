	...
	$.PullToRefresh = $.Class.extend({
			...
			if($.isFunction(self.options.down.callback)) {
				self.pullDownTips = (function() {
					...
					if(!element) {
						...
						element.innerHTML = '<div class="mui-pull-top-wrapper"><span class="mui-pull-loading mui-icon mui-icon-pulldown"></span><span style="position:relative;top:5px;left:10px;">' + self.options.down.contentinit + '</span></div>';
						element.addEventListener('webkitTransitionEnd', self);
					}
					self.pullDownTipsIcon = element.querySelector(SELECTOR_PULL_LOADING);
					self.element.insertBefore(element, self.element.querySelector("*"));
					return element;
				}());
			}
		},
			...
			if($.isFunction(self.options.up.callback)) {
				self.pullUpTips = (function() {
					...
					if(!element) {
						...
						element.innerHTML = '<div class="mui-pull-bottom-wrapper"><span class="mui-spinner mui-hidden"></span><span class="mui-pull-loading">' + self.options.up.contentinit + '</span></div>';
						self.element.querySelector("div").appendChild(element);
					}
					self.pullUpTipsLoadding = element.querySelector(".mui-spinner");
		...
		_drag: function(e) {
			...
			if(this.isDragging) {
				...
				var state = (deltaY > this.options.down.height && e.currentTarget.scrollTop == 0) ? STATE_AFTERCHANGEOFFSET : STATE_BEFORECHANGEOFFSET;
				if(this.state !== state) {
					this.state = state;
		...
		pulling: function(deltaY) {
			//this.pullDownTips.style.paddingTop = (deltaY / 2) + 'px';
			//this.pullDownTips.style.paddingBottom = (deltaY / 2) + 'px';
			...
			this.pullDownTips.nextSibling.style.webkitTransform = 'translate3d(0,' + deltaY + 'px,0)';
		},
		beforeChangeOffset: function(deltaY) {
			this.pullDownTipsIcon.className = CLASS_PULL_TOP_ARROW;
			this.pullDownTips.getElementsByTagName("span")[1].innerHTML = this.options.down.contentinit;
		},
		afterChangeOffset: function(deltaY) {
			this.pullDownTipsIcon.className = CLASS_PULL_TOP_ARROW_REVERSE;
			this.pullDownTips.getElementsByTagName("span")[1].innerHTML = this.options.down.contentdown;
		},
		...
		pullDownLoading: function() {
			...
			this.pullDownTips.getElementsByTagName("span")[1].innerHTML = this.options.down.contentrefresh;
		...
		pullUpLoading: function(e) {
			...
			this.pullUpTipsLoadding.classList.remove("mui-hidden");
		...
		endPullDownToRefresh: function() {
			...
			this.pullDownTips.nextSibling.style.webkitTransform = 'translate3d(0,0,0)';
			if(this.deltaY <= 0 || !this.deltaY) {
				this.removePullDownTips();
			} else {
				this.removing = true;
			}
		...
		endPullUpToRefresh: function(finished) {
			if(finished) {
				this.finished = true;
				this.pullUpTipsLoadding.classList.add("mui-hidden");
