app.controller("ContactOrgController", ["$scope", "ApiService", "DataService", "Loading", "RPCObserver", "UtilsService", function($scope, ApiService, DataService, Loading, RPCObserver, UtilsService) {
	...
	//获取设备目录
	function getDirList() {
		var url = ApiService.Api52 + "/api/v1/prod/GetPkgClassExt?pid=" + $scope.catalogID;
	...
	function getEqInfoList(type) {
		var url = ApiService.Api52 + "/api/v1/prod/GetProdListByDir?pageIndex=" + pageIndex + "&pageSize=" + pageSize + "&dirId=" + $scope.catalogID;
		DataService.get(url).then(function(res) {
			$scope.Network = true;
			Loading.hide();
			if(res) {
