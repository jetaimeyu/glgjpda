    ...
    var ImageViewer = $.ImageViewer = $.Class.extend({
        ...
        findAllImage: function () {
            ...
            if (self.images.length == 1) {
                self.leftButton &&  (self.leftButton.style.display = "none");
                self.rightButton && (self.rightButton.style.display = "none");
            }
            else {
                self.leftButton &&  (self.leftButton.style.display = "block");
                self.rightButton && (self.rightButton.style.display = "block");
            }
        },
        //检查图片是否为启动预览的图片
        ...
        bindEvent: function () {
            ...
            //处理左右按钮
            self.leftButton.addEventListener(tapEventName, function (event) {
                self.prev();
                event.preventDefault();
                event.stopPropagation();
            }, false);
            self.rightButton.addEventListener(tapEventName, function (event) {
                self.next();
                event.preventDefault();
                event.stopPropagation();
            }, false);
        ...
        createImage: function (index, type) {
            ...
            item.appendChild($.dom('<span><img src="' + self.images[self.index].getAttribute("data-preview-src") + '"/></span>')[0]);
