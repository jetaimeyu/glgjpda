app.controller("searchCtrl", ["$scope", "ApiService", "DataService", "UtilsService", "Loading", "CacheService", "AuthService", "RPCObserver", "$timeout", function($scope, ApiService, DataService, UtilsService, Loading, CacheService, AuthService, RPCObserver, $timeout) {
	...
	$scope.search = {
		...
		history: [],
		getHistory: function() {
			var temp = CacheService.get(userId + "equ_warehouse_search");
			if(temp)
				$scope.search.history = CacheService.get(userId + "equ_warehouse_search");
			$scope.domShowX = true;
			$scope.domShow = false;
		},
		doSearch: function() {
			...
			CacheService.set(userId + "equ_warehouse_search", $scope.search.history, true);
			//开启上拉事件
	...
	function getPagedCompEqRepairRecordsList() {
		var url = ApiService.Api52 + "/api/v1/EqApply/GetWarehousePageList?eqName=" + encodeURIComponent($scope.search.text) + "&pageIndex=" + $scope.pageIndex + "&pageSize=" + $scope.pageSize;
	...
	RPCObserver.on('refresh_equ_whrahouse_list', 'refresh_equ_whrahouse_list');
	window.refresh_equ_whrahouse_list = function() {
		$scope.search.doSearch();
	};
	...
	$scope.equSelect = function(equ) {
		if($scope.action == 'select') {
			...
			if(view.opener().callback) {
				//				console.log(JSON.stringify(view.opener().callback)+equ)
				opener.evalJS(view.opener().callback + "(" + JSON.stringify(equ) + ")");
			}
