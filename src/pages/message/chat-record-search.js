...
var dom = {
	...
	webView: {
		...
		initParams: function() {
			dom.properties.scope.search.chatID = parseInt(this.cur.chatID);
			dom.properties.scope.search.chatName = this.cur.chatName;
			dom.properties.scope.search.chatCompID = this.cur.chatCompID;
			dom.properties.scope.search.chatLogo = this.cur.chatLogo;
		}
	},
	search: {
		init: function() {
...
var curData = {
	search: {
		...
		doSearch: function() {
			...
			var url = dom.properties.apiHost + "/api/v1/Message/MsgRecord_Search?oppoId=" + dom.properties.scope.search.chatID + "&pageSize=" + this.count + "&pageIndex=" + this.page + "&sort=2&q=" + encodeURIComponent(dom.properties.scope.search.text);
