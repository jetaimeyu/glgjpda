...
app.controller("scanProdsCtl", ["$scope", "ApiService", "DataService", "Loading", "DatePickerService", "$Modal", "UtilsService", "$filter", "AuthService", function($scope, ApiService, DataService, Loading, DatePickerService, modal, UtilsService, $filter, AuthService) {
	...
	window.OpenList = function() {
		...
		var _starttime = angular.copy(_date.startDate);
		var _endtime = angular.copy(_date.endDate);

		if(_date.curType == 'month' && _date.curDay != '整月') {
			_starttime = new Date(_starttime.setDate(_date.curDay));
			_endtime = new Date(angular.copy(_starttime).addDays(1));
		}

		UtilsService.openWindow({
			needLogin: true,
			id: 'produce-list.html',
			url: 'produce-list.html?province=' + encodeURI(_province) + "&city=" + encodeURI(_city) + "&starttime=" + encodeURI(_starttime.Format("yyyy-MM-dd 00:00:00")) + "&endtime=" + encodeURI(_endtime.Format("yyyy-MM-dd 00:00:00"))
		});
	...
	var curData = {
		getData: function() {
			...
			var url = ApiService.Api50 + "/api/v1/Equipment/GetMapEqInfoAllState?starttime=" + startDate.Format("yyyy-MM-dd 00:00:00") + "&endtime=" + endDate.Format("yyyy-MM-dd 00:00:00") + "&state=0";
